        ...
        <CardContent className="relative p-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-bold text-white">High-Load Buses</h3>
                  <Badge className="bg-red-500/20 text-red-400 border border-red-500/30 text-[9px] px-1.5 py-0 font-bold">
                    <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse mr-1 inline-block" />
                    LIVE
                  </Badge>
                </div>
                <p className="text-[11px] text-zinc-400">
                  {totalStudents} students over capacity • {overloadedBuses.length} buses need attention
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white shadow-lg shadow-purple-500/20 h-8 text-xs px-3 font-semibold"
            >
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <motion.div
              ...
              className="bg-gradient-to-br from-red-500/10 to-red-900/20 border border-red-500/30 rounded-xl p-3 cursor-pointer"
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-semibold text-red-400 uppercase tracking-wide">Critical</span>
                <div className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center">
                  <Zap className="w-3 h-3 text-red-400" />
                </div>
              </div>
              <div className="text-2xl font-bold text-red-400">{criticalCount}</div>
              <div className="text-[9px] text-red-400/60">Both shifts overloaded</div>
            </motion.div>

            {/* High */}
            <motion.div
              whileHover={{ scale: 1.02 }}
              className="bg-gradient-to-br from-amber-500/10 to-amber-900/20 border border-amber-500/30 rounded-xl p-3 cursor-pointer"
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-semibold text-amber-400 uppercase tracking-wide">High</span>
                <div className="w-5 h-5 rounded-full bg-amber-500/20 flex items-center justify-center">
                  <TrendingUp className="w-3 h-3 text-amber-400" />
                </div>
              </div>
              <div className="text-2xl font-bold text-amber-400">{highCount}</div>
              <div className="text-[9px] text-amber-400/60">Single shift overloaded</div>
            </motion.div>

            {/* Total Affected */}
            <motion.div
              whileHover={{ scale: 1.02 }}
              className="bg-gradient-to-br from-cyan-500/10 to-cyan-900/20 border border-cyan-500/30 rounded-xl p-3 cursor-pointer"
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-semibold text-cyan-400 uppercase tracking-wide">Total</span>
                <div className="w-5 h-5 rounded-full bg-cyan-500/20 flex items-center justify-center">
                  <Bus className="w-3 h-3 text-cyan-400" />
                </div>
              </div>
              <div className="text-2xl font-bold text-cyan-400">{overloadedBuses.length}</div>
              <div className="text-[9px] text-cyan-400/60">Buses overloaded</div>
            </motion.div>
          </div>
              ...
              <AnimatePresence>
                {overloadedBuses.map((bus, index) => (
                  <motion.div
                    ...
                    className={cn(
                      "flex-shrink-0 w-64 rounded-xl border p-3 cursor-pointer transition-all duration-200",
                      bus.reason === 'both'
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-zinc-400">{bus.routeName}</div>
                        </div>
                      </div>
                      <Badge className={cn(
          ...
          {/* Footer */}
          <div className="flex items-center justify-between text-[10px] text-zinc-400 mt-3 pt-3 border-t border-zinc-700/50">
