...
export default function AdminFeedbackPage() {
    ...
    const [adminMessage, setAdminMessage] = useState('Kindly look after this issue.');
    ...
    // Load moderators and drivers for forward modal
    const loadRecipients = useCallback(async () => {
        ...
        try {
            ...
            // Fetch moderators
            const modResponse = await fetch('/api/moderators/get-all', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (modResponse.ok) {
                const modData = await modResponse.json();
                if (modData.success) {
                    setModerators(modData.moderators.map((m: any) => ({
                        id: m.moderatorUid || m.id,
                        name: m.name || m.fullName || 'Unknown',
                        employeeId: m.empId || m.employeeId || 'N/A',
            ...
            if (driverResponse.ok) {
                ...
                if (driverData.success) {
                    const mappedDrivers = driverData.drivers.map((d: any) => ({
                        ...
                        employeeId: d.driverId || d.employeeId || 'N/A',
    ...
    const handleOpenForwardModal = (item: FeedbackEntry) => {
        ...
        setAdminMessage('Kindly look after this issue.');
    ...
    const handleSendForward = async () => {
        ...
        try {
            ...
            const sender = {
                ...
                userName: userData.name || userData.fullName || 'Admin',
            ...
            addToast(`Notification sent successfully to ${selectedRecipients.length} ${selectedRecipients.length === 1 ? 'recipient' : 'recipients'}!`, 'success');
        ...
        <div className="mt-12 space-y-6">
                    ...
                    {!loading && filteredFeedback.length > 0 && (
                        <div className="space-y-4">
                            {filteredFeedback.map((item) => (
                                <div
                                    key={item.id}
                                    className="bg-[#1c2128] border border-gray-700/60 rounded-2xl p-5 transition-all duration-200 hover:border-indigo-500/40 hover:shadow-lg hover:shadow-indigo-500/5"
                                >
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="relative">
                                                <div className="h-12 w-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center overflow-hidden border-2 border-gray-600">
                                                    {item.profile_url ? (
                                                        <Image
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {item.read ? null : (
                                                        <Badge className="bg-indigo-500/20 text-indigo-400 border-0 text-[10px] font-semibold px-2 py-0.5 animate-pulse">
                                                            NEW
                                                        </Badge>
                                                    )}
                                                </div>
            ...
            <Dialog open={viewModalOpen} onOpenChange={setViewModalOpen}>
                <DialogContent
                    ...
                    className="max-w-[95vw] md:max-h-[95vh] sm:max-w-lg h-[90vh] sm:h-auto p-0 gap-0 rounded-2xl overflow-hidden bg-[#0A0C10] border-gray-800 shadow-2xl flex flex-col mt-4 md:mt-4"
                >
                    {selectedFeedback && (
                        <>
                            {/* Top Accent Line */}
            ...
            <Dialog open={forwardModalOpen} onOpenChange={setForwardModalOpen}>
                ...
                >
                    <DialogHeader className="px-5 sm:px-6 pt-5 pb-3 border-b border-gray-800 bg-gradient-to-br from-blue-500/10 to-transparent">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-base font-bold text-white">Send Notification</DialogTitle>
                                <DialogDescription className="text-xs text-gray-400">
                                    Notify Moderators and Drivers about this feedback
                                </DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {selectedRecipients.length > 0 ? (
                                                ...
                                                <span className="text-slate-500 font-medium">
                                                    {loadingRecipients ? "Loading staff list..." : "Select team members..."}
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        >
                                            <CommandEmpty className="py-6 text-center">
                                                <div className="flex flex-col items-center gap-2">
                                                    ...
                                                    <p className="text-sm text-slate-600">No team members found</p>
                                                </div>
                                            </CommandEmpty>

                                            <CommandGroup heading={<span className="text-[10px] font-bold text-indigo-500/80 uppercase tracking-widest px-2 mb-2 block">Moderators</span>}>
                                                {moderators.map((mod) => (
                                                    <CommandItem
                                                        key={mod.id}
                                                        value={mod.name + mod.employeeId}
                                                        onSelect={() => {
                                                        ...
                                                        className="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all duration-200 aria-selected:bg-indigo-600/20 group hover:bg-slate-800"
                                                    >
                                                        <div className={`h-10 w-10 rounded-full flex items-center justify-center border transition-all duration-200 ${selectedRecipients.includes(mod.id) ? 'bg-indigo-600 border-indigo-500 shadow-lg shadow-indigo-600/30' : 'bg-slate-800 border-slate-700 group-hover:border-slate-600'}`}>
                                                            <ShieldCheck className={`h-5 w-5 ${selectedRecipients.includes(mod.id) ? 'text-white' : 'text-indigo-400'}`} />
                                                        </div>
                                                        <div className="flex flex-col flex-1 min-w-0">
                                                            <span className={`font-semibold truncate text-sm ${selectedRecipients.includes(mod.id) ? 'text-white' : 'text-slate-200'}`}>{mod.name}</span>
                                                            <span className="text-[11px] text-slate-500/80 font-medium">{mod.employeeId}</span>
                                                        </div>
                                                        {selectedRecipients.includes(mod.id) && (
                                                            <div className="h-5 w-5 rounded-full bg-indigo-500 flex items-center justify-center border-2 border-slate-950">
                        ...
                        <div className="space-y-3">
                            ...
                            <Textarea
                                ...
                                className="min-h-[160px] sm:min-h-[80px] text-sm resize-none bg-[#1c2128] border-gray-700/50 text-gray-200 placeholder:text-gray-500 rounded-xl"
                            />
