...
export default function PrivacyPolicyPage() {
  ...
  useEffect(() => {
    const fetchPrivacy = async () => {
      try {
        const response = await fetch('/api/settings/privacy-config');
        if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#08090C] text-slate-400 font-sans selection:bg-indigo-500/20">
      ...
      {/* Main Content */}
      <main className="relative z-10 max-w-5xl mx-auto px-6 md:py-15 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {/* Header */}
        ...
        <div className="mt-40 pt-16 border-t border-white/[0.04] pb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
          ...
          <div className="text-left md:text-right space-y-4">
            <nav className="flex flex-wrap md:justify-end gap-6 text-[10px] font-bold text-slate-600 uppercase tracking-widest">
              ...
              <Link href="/privacy-policy" className="text-indigo-400 underline decoration-indigo-500/30 decoration-2 underline-offset-4">Policy</Link>
