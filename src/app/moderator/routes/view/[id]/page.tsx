...
export default function ViewRoutePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleEdit = () => {
    router.push(`/moderator/routes/edit/${id}`);
  };
    ...
    return (
      <div className="mt-15 min-h-screen flex items-center justify-center bg-[#010717]">
        <div className="text-center">
          ...
          <Link href="/moderator/routes" className="text-purple-400 hover:text-purple-300 mt-4 inline-block">
            ← Back to Routes
          </Link>
        </div>
      </div>
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 min-w-0">
                  ...
                  <div className="flex items-center space-x-4 w-full sm:w-auto">
                    <div className="relative flex-shrink-0">
                      <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-emerald-500 to-blue-600 flex items-center justify-center shadow-lg border-2 border-emerald-400/30">
                        <MapPin className="h-8 w-8 text-white" />
                      </div>
                      <div className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center shadow-md">
                        <span className="text-xs font-bold text-white">{route.stops?.length || 0}</span>
                      </div>
                    </div>
