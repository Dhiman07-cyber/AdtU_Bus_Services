...
export default function EnhancedModeratorDashboard() {
  ...
  const fetchRealTotalCounts = async () => {
    try {
      ...
      // 5. Financial Overview & Payments
      let last30DaysPayments = 0;
      try {
        if (currentUser) {
          ...
          if (data.success && data.data) {
            last30DaysPayments = data.data.completedPayments;
          }
        }
      } catch (e) {
  ...
  // Let's use the new allRoutes state for charts.
  ...
  const calculateStats = useCallback(() => {
    ...
    setStats({
      ...
      pendingVerifications: realCounts.pendingVerifications,
      ...
      renewalRequests: realCounts.renewalRequests,
      last30DaysPayments: realCounts.last30DaysPayments
    });
  }, [students, drivers, allBuses, allRoutes, applications, notifications, realCounts]);
  ...
  const calculateCharts = useCallback(() => {
    ...
    ].filter(i => true)); // Keep both visible
    ...
    const routeOccupancyData = routesToUse.map((route: any) => {
      ...
      const routeBuses = allBuses.length > 0
        ? allBuses.filter((b: any) => b.routeId === route.routeId || b.route?.routeId === route.routeId)
        : buses.filter((b: any) => b.routeId === route.routeId || b.route?.routeId === route.routeId);
    ...
    <div className="flex-1 bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30 dark:from-[#05060e] dark:via-[#0a0d1f] dark:to-[#05060e] min-h-screen relative overflow-hidden">
      ...
      <div className="px-6 md:px-12 pt-20 md:pt-20 pb-20 md:pb-20 space-y-3 relative z-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-5">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              <div>
                <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-green-600 via-teal-600 to-cyan-600 bg-clip-text text-transparent animate-pulse">
                ...
                <div className="text-[12px] text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                  ...
                  Moderation Console • Last updated: {lastUpdated.toLocaleTimeString()}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
          <Card className="hover:scale-[1.02] transition-all duration-300 bg-gradient-to-br from-fuchsia-500/15 to-pink-500/20 border-fuchsia-500/30 dark:border-fuchsia-500/40 backdrop-blur-md shadow-sm group cursor-pointer">
          ...
          <Card
            ...
            onClick={() => handleKpiClick('/moderator/students')}
          >
            <CardHeader className="px-2.5">
              <div className="flex items-center justify-between">
            ...
            <CardContent className="px-2.5">
              <div className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{stats.totalStudents}</div>
              <p className="text-[8px] text-gray-500 mt-0.5">Enrolled users</p>
            </CardContent>
          </Card>

          <Card className="hover:scale-[1.02] transition-all duration-300 bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-500/20 dark:to-teal-500/20 border-emerald-200 dark:border-emerald-500/30 backdrop-blur-sm shadow-sm group cursor-pointer">
            <CardHeader className="px-2.5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[10px] font-medium text-gray-500 dark:text-gray-400 group-hover:text-green-400 transition-colors !pb-0 !mb-0">Active Students</CardTitle>
                <Activity className="h-3 w-3 text-green-500 group-hover:scale-110 transition-transform" />
              </div>
            </CardHeader>
            <CardContent className="px-2.5">
              <div className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-green-400 transition-colors">{stats.activeStudents}</div>
              <p className="text-[8px] text-gray-500 mt-0.5">Service active</p>
            </CardContent>
          </Card>

          <Card
            className="cursor-pointer hover:scale-[1.02] transition-all duration-300 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-500/20 dark:to-amber-500/20 border-orange-200 dark:border-orange-500/30 backdrop-blur-sm shadow-sm group"
            onClick={() => handleKpiClick('/moderator/drivers')}
          >
            <CardHeader className="px-2.5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[10px] font-medium text-gray-500 dark:text-gray-400 group-hover:text-orange-400 transition-colors !pb-0 !mb-0">Active Drivers</CardTitle>
                <UserCheck className="h-3 w-3 text-orange-500 group-hover:scale-110 transition-transform" />
              </div>
            </CardHeader>
            <CardContent className="px-2.5">
              <div className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-orange-400 transition-colors">{stats.activeDrivers}</div>
              <p className="text-[8px] text-gray-500 mt-0.5">On duty today</p>
            </CardContent>
          </Card>

          <Card
            className="cursor-pointer hover:scale-[1.02] transition-all duration-300 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-500/20 dark:to-pink-500/20 border-purple-200 dark:border-purple-500/30 backdrop-blur-sm shadow-sm group"
            onClick={() => handleKpiClick('/moderator/applications')}
          >
            <CardHeader className="px-2.5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[10px] font-medium text-gray-500 dark:text-gray-400 group-hover:text-purple-400 transition-colors !pb-0 !mb-0">Pending Applications</CardTitle>
                <FileText className="h-3 w-3 text-purple-500 group-hover:scale-110 transition-transform" />
              </div>
            </CardHeader>
            <CardContent className="px-2.5">
              <div className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-purple-400 transition-colors">{stats.pendingApplications}</div>
        ...
        <HighLoadAlert role="moderator" className="animate-fade-in" />
        ...
        {/* Active Trips & Quick Actions - Ultra Compact */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <Card className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-pink-500/30 transition-all duration-300 group cursor-pointer">
            ...
            <CardContent className="px-2.5 min-h-[160px]">
          ...
          <Card className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-yellow-500/30 transition-all duration-300 group cursor-pointer">
            ...
            <CardContent className="px-2.5 min-h-[140px]">
