...
export async function GET(request: NextRequest) {
  try {
    ...
    // Rate limit
    const rl = applyRateLimit(createRateLimitId(auth.uid, 'routes-list'), RateLimits.READ);
    if (!rl.allowed) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Rate limit
    const rl = applyRateLimit(createRateLimitId(auth.uid, 'routes-create'), RateLimits.CREATE);
    if (!rl.allowed) {
      return NextResponse.json({ error: 'Too many requests' }, { status: 429, headers: rl.headers });
    }
