    ...
    if (useAdminSDK && auth && db) {
      try {
        ...
        if (adminUserData.role !== 'admin' && adminUserData.role !== 'moderator') {
          return new Response(JSON.stringify({
            success: false,
            error: 'Unauthorized: Only admins and moderators can delete students'
          }), {
            status: 403,
            headers: { 'Content-Type': 'application/json' },
          });
        ...
        console.log(`Successfully deleted student ${uid} and all associated data`);
      ...
      return new Response(JSON.stringify({
        success: false,
        error: 'Admin SDK not available for student deletion'
      }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
