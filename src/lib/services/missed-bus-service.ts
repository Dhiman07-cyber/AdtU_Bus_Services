...
export class MissedBusService {
  ...
  /**
   * Expire pending requests that have passed their TTL
   * This is called by:
   * 1. Daily cron job (fallback)
   * 2. On-request cleanup during API calls (primary method for Vercel Hobby plan)
   * 
  ...
  async expirePendingRequests(): Promise<number> {
    try {
      ...
      const { data: expiredRequests, error } = await this.supabase
        ...
        .update({
          status: 'expired',
          student_location: null // Clean up location data
        })
        .eq('status', 'pending')
        .lt('expires_at', now)
        .select('id');
  ...
  /**
   * Perform eager cleanup - should be called at the start of key API operations
   * This is the primary cleanup mechanism since Vercel Hobby plan doesn't allow frequent crons
   */
  async performEagerCleanup(): Promise<void> {
    try {
      // Run expiry check in the background
      await this.expirePendingRequests();
    } catch (error) {
      // Don't let cleanup errors affect the main operation
      console.warn('Eager cleanup warning:', error);
    }
  }

  /**
   * Get student's active request status (with eager cleanup)
   * Used by the /api/missed-bus/status endpoint
   */
  async getStudentRequestStatus(studentId: string): Promise<any | null> {
    // Run eager cleanup first - this is the primary cleanup mechanism!
    await this.performEagerCleanup();
  ...
  /**
   * Get pending missed-bus requests for a driver (with eager cleanup)
   * Used by the /api/missed-bus/driver-requests endpoint
   */
  async getPendingRequestsForDriver(driverId: string): Promise<any[]> {
    ...
    try {
      // First, get the driver's active trip
      const { data: driverStatus } = await this.supabase
        .from('driver_status')
        .select('*')
        .eq('driver_uid', driverId)
      ...
      if (!driverStatus) {
        return []; // Driver not on an active trip
      }

      // Get all pending requests for this route that have the driver's bus as a candidate
      const { data: requests, error } = await this.supabase
        .from('missed_bus_requests')
        .select('*')
        .eq('route_id', driverStatus.route_id)
        .eq('status', 'pending')
      ...
      // Filter to only include requests where this driver's bus is a candidate
      const filteredRequests = requests.filter((req: any) => {
        const candidates = req.trip_candidates || [];
        return candidates.some((c: any) =>
          c.bus_id === driverStatus.bus_id ||
      ...
      console.error('Error getting pending requests for driver:', error);
      return [];
    }
  }
}
